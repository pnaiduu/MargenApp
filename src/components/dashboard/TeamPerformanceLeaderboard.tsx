...
function Stars({ value }: { value: number | null }) {
  ...
  return (
    <span className="text-[var(--color-margen-text)]" title={`${value} / 5`}>
      {'★'.repeat(full)}
...
export function TeamPerformanceLeaderboard() {
  ...
  return (
    <section className="rounded-xl border border-[var(--color-margen-border)] bg-[var(--color-margen-surface-elevated)]">
      <div className="border-b border-[var(--color-margen-border)] px-5 py-4">
        <h2 className="section-title">Team Performance</h2>
        <p className="mt-1 text-sm text-[var(--color-margen-muted)]">Leaderboard for this week · sorted by jobs completed</p>
      </div>
      ...
      {loading ? (
        ...
        <p className="px-5 py-10 text-center text-sm text-[var(--color-margen-muted)]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--color-margen-border)] text-xs font-semibold uppercase tracking-wide text-[var(--color-margen-muted)]">
            ...
            <tbody>
              {rows.map((r) => (
                <motion.tr
                  key={r.technicianId}
                  className="border-b border-[#f0f0f0] last:border-0 hover:bg-[var(--color-margen-hover)]"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.2, ease: easePremium }}
                >
                  <td className="px-4 py-3 font-semibold tabular-nums text-[var(--color-margen-text)]">#{r.rank}</td>
                  <td className="px-4 py-3">
                    <Link
                      to={`/technicians/${r.technicianId}`}
                      className="flex items-center gap-3 font-medium text-[var(--color-margen-text)] hover:text-[var(--margen-accent)]"
                    >
